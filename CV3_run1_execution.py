...
Execution script to initialise, train, validate, and produce test output for
the KNN classifier.

Companion script is CV3_run1_functions.py
"""
...
# creating an array of the rest images
...
# initialisting classifier with K=24, training and predicting    
TestKNN = KNN_Classifier(24)
TestKNN.train(knn_input, knn_target)
results = TestKNN.test(knn_test_input)

# using the dictionary to label the outputs correctly
label_results = []
...
# creating a plot of the validation results
k = np.arange(1,101).reshape(100,1)
fig=plt.figure()
plt.scatter(k, val_matrix[:,0],3, marker="x")
plt.scatter(k, val_matrix[:,1],3, marker="o")
plt.scatter(k, val_matrix[:,2],3, marker="s")
plt.scatter(k, val_matrix[:,3],3, marker="2")
plt.scatter(k, val_matrix[:,4],3, marker="d")
plt.plot(k, np.mean(val_matrix, axis=1))
fig.suptitle('KNN precision at validation', fontsize=20)
plt.xlabel('Number of nearest neighbours', fontsize=16)
plt.ylabel('Average precision', fontsize=16)
plt.show()
fig.savefig('KNN_validation_results.jpg')